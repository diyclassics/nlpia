...
MAX_LEN_FILEPATH = (
    1023  # on OSX `open(fn)` raises OSError('Filename too long') if len(fn)>=1024
)
...
DICTABLE_TYPES = (
    ...
    tuple,
    list,
)  # convertable to a dictionary (inherits Mapping or is a list of key/value pairs)
...
HIST_NAME = {
    ...
    "pmf": "pmf",
    "pdf": "pmf",
    "pd": "pmf",  # noqa  prob. mass/density function, prob. density
    "cmf": "cmf",
    "cdf": "cmf",  # noqa
    "cfd": "cfd",
    "cff": "cfd",
...
MIN_TIMESTAMP = pd.Timestamp("1677-09-22 00:12:44", tz="utc")
...
NULL_VALUES = set(
    ["0", "None", "null", "'"] + ["0." + z for z in ["0" * i for i in range(10)]]
)
