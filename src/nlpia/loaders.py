...
ZIP_PATHS = [
    [os.path.join(BIGDATA_PATH, fn) for fn in ZIP_FILES[k]] if ZIP_FILES[k] else k
    for k in ZIP_FILES.keys()
]
...
def load_imdb_df(
    dirpath=os.path.join(BIGDATA_PATH, "aclImdb"),
    subdirectories=(("train", "test"), ("pos", "neg", "unsup")),
):
    ...
    for subdirs in tqdm(list(product(*subdirectories))):
        ...
        if not os.path.isfile(urlspath):
            if subdirs != (
                "test",
                "unsup",
            ):  # test/ dir doesn't usually have an unsup subdirectory
                log.warning(
                    "Unable to find expected IMDB review list of URLs: {}".format(
                        urlspath
                    )
                )
            continue
        ...
        if not os.path.isdir(textsdir):
            log.warning(
                "Unable to find expected IMDB review text subdirectory: {}".format(
                    textsdir
                )
            )
            continue
        ...
        df["index0"] = subdirs[
            0
        ]  # TODO: column names more generic so will work on other datasets
        df["index1"] = subdirs[1]
...
def load_anki_df(language="deu"):
    ...
    if os.path.isfile(language):
        ...
        filepath = os.path.join(
            BIGDATA_PATH, "{}-eng".format(lang), "{}.txt".format(lang)
        )
    df = pd.read_table(filepath, skiprows=1, header=None)
    for i, newc in enumerate(["eng", lang, "license"]):
        df.columns = [
            newc if str(c).lower().strip().startswith(newc) else c for c in df.columns
        ]
...
for yr in range(2011, 2017):
    BIG_URLS["cdc" + str(yr)[-2:]] = (
        "https://www.cdc.gov/brfss/annual_data/{yr}/files/LLCP{yr}ASC.zip".format(
            yr=yr
        ),
        None,
    )
...
BIG_URLS["cornell"] = BIG_URLS["cornellmoviedialog"] = BIG_URLS[
    "cornellmoviedialogs"
] = BIG_URLS["cornell_movie_dialog"] = BIG_URLS["cornell_movie_dialogs"] = BIG_URLS[
    "cornell_movie_dialog_corpus"
] = BIG_URLS[
    "cornell_movie_dialogs_corpus"
] = BIG_URLS[
    "cornellmovies"
...
BIG_URLS["glove"] = BIG_URLS["glovesm"] = BIG_URLS["glove-sm"] = BIG_URLS[
    "glove_sm"
] = BIG_URLS["glove-small"] = BIG_URLS["glove_small"]
BIG_URLS["ubuntu"] = BIG_URLS["ubuntu_dialog"] = BIG_URLS["ubuntu_dialog_1500k"]
BIG_URLS["glovelg"] = BIG_URLS["glove_lg"] = BIG_URLS["glove-lg"] = BIG_URLS[
    "glove-large"
] = BIG_URLS["glove_large"]
BIG_URLS["glovemed"] = BIG_URLS["glove_med"] = BIG_URLS["glove-med"] = BIG_URLS[
    "glove-medium"
...
ANKI_LANGUAGES = (
    "afr arq ara aze eus bel ben ber bul yue cat cbk cmn chv hrv ces dan nld est fin fra glg kat "
...
ANKI_LANGUAGE_SYNONYMS = list(
    zip(
        "fre esp ger french spanish german turkish turkey dut dutch".split(),
...
for lang in ANKI_LANGUAGES:
    for eng in ENGLISHES:
        BIG_URLS[lang] = (
            "http://www.manythings.org/anki/{}-eng.zip".format(lang),
            1000,
            "{}-{}".format(lang, eng),
            load_anki_df,
        )
...
for name in GOOGLE_NGRAM_NAMES:
    BIG_URLS["1gram_{}".format(name)] = (
        GOOGLE_NGRAM_URL + GOOGLE_NGRAM_FILE.format(name),
        1000,
        GOOGLE_NGRAM_FILE.format(name),
        pd.read_table,
        {
            "sep": "\t",
            "header": None,
            "names": "term_pos year term_freq book_freq".split(),
        },
    )
...
try:
    ...
    log.warning(
        "Found BIGDATA index in {default} so it will overwrite nlpia.loaders.BIG_URLS !!!".format(
    ...
    log.info(
        'No BIGDATA index found in {default} so copy {latest} to {default} if you want to "freeze" it.'.format(
            default=BIGDATA_INFO_FILE, latest=BIGDATA_INFO_LATEST
        )
    )
BIG_URLS.update(
    dict(zip(BIGDATA_INFO.name, zip(BIGDATA_INFO.url, BIGDATA_INFO.file_size)))
)
BIGDATA_INFO = pd.DataFrame(
    list(
        zip(
            BIG_URLS.keys(),
            list(zip(*BIG_URLS.values()))[0],
            list(zip(*BIG_URLS.values()))[1],
        )
    ),
...
DATA_NAMES = {
    ...
    "hutto_tweets0": os.path.join(
        DATA_PATH, "hutto_ICWSM_2014/tweets_GroundTruth.csv.gz"
    ),
    "hutto_tweets": os.path.join(DATA_PATH, "hutto_ICWSM_2014/tweets_GroundTruth.csv"),
    "hutto_nyt": os.path.join(
        DATA_PATH, "hutto_ICWSM_2014/nytEditorialSnippets_GroundTruth.csv.gz"
    ),
    "hutto_movies": os.path.join(
        DATA_PATH, "hutto_ICWSM_2014/movieReviewSnippets_GroundTruth.csv.gz"
    ),
    "hutto_products": os.path.join(
        DATA_PATH, "hutto_ICWSM_2014/amazonReviewSnippets_GroundTruth.csv.gz"
    ),
}

# FIXME: put these in BIG_URLS, and test/use them with get_data()
DDL_DS_QUESTIONS_URL = (
    "http://minimum-entropy.districtdatalabs.com/api/questions/?format=json"
)
DDL_DS_ANSWERS_URL = (
    "http://minimum-entropy.districtdatalabs.com/api/answers/?format=json"
)
...
def rename_file(source, dest):
    ...
    if not isinstance(source, str):
        ...
        return [
            rename_file(s, d)
            for (s, d) in zip_longest(
                source, dest, fillvalue=[source, dest][int(len(source) > len(dest))]
            )
        ]
...
def series_strip(
    series, startswith=None, endswith=None, startsorendswith=None, ignorecase=True
):
    ...
    if not (startsorendswith or endswith or startswith):
        log.warning(
            "In series_strip(): You must specify endswith, startswith, or startsorendswith string arguments."
        )
        return series
    if startsorendswith:
...
def get_longest_table(
    url="https://www.openoffice.org/dev_docs/source/file_extensions.html", header=0
):
...
def get_leet_map():
    ...
    df = get_longest_table(
        "https://sites.google.com/site/inhainternetlanguage/different-internet-languages/l33t/list-of-l33ts",
        header=None,
    )
...
def longest_table(dfs):
    ...
    sorted_indices = sorted(
        (len(df if hasattr(df, "__len__") else []), i) for i, df in enumerate(dfs)
    )
    return dfs[sorted_indices[-1][1]]


def get_filename_extensions(
    url="https://tan.sfo2.digitaloceanspaces.com/midata/public/corpora/nlpia/file_extensions.html",
):
...
def unzip(filepath, verbose=True):
    ...
    for f in tqdm_prog(os.listdir(unzip_dir)):
        if f[-1] in " \t\r\n\f":
            ...
            log.warning(
                "Stripping whitespace from end of filename: {} -> {}".format(
    ...
    w2v_paths = [
        os.path.join(BIGDATA_PATH, f[:-4] + ".w2v.txt")
        for f in os.listdir(unzip_dir)
    ...
    for f, word2vec_output_file in zip(os.listdir(unzip_dir), w2v_paths):
        ...
        log.info(
            "Attempting to converting GloVE format to Word2vec: {} -> {}".format(
        ...
        try:
            ...
            log.info(
                "Failed to convert GloVE format to Word2vec: {} -> {}".format(
                    repr(glove_input_file), repr(word2vec_output_file)
                )
            )

    txt_paths = [
        os.path.join(BIGDATA_PATH, f.lower()[:-4] + ".txt")
        for f in os.listdir(unzip_dir)
    ...
    for f, txt_file in zip(os.listdir(unzip_dir), txt_paths):
        if f.lower().endswith(".asc"):
            ...
            log.info(
                "Renaming .asc file to .txt: {} -> {}".format(
...
def download_unzip(names=None, normalize_filenames=False, verbose=True):
    ...
    for name in names:
        ...
        if name in BIG_URLS:
            ...
            log.debug(
                "downloaded name={} to filepath={}".format(name, file_paths[name])
            )
            ...
            df = pd.read_html(
                DATA_INFO["url"][name], **DATA_INFO["downloader_kwargs"][name]
            )[-1]
...
def download_file(
    ...
    if (
        not remote_size
        or not stat["type"] == "file"
        or not local_size >= remote_size
        or not stat["size"] > MIN_DATA_FILE_SIZE
    ):
        try:
            ...
            log.warning(
                "This can happen for Google Word Vector download links to Dropbox or Google Docs."
            )
    try:
    ...
    if (
        ...
        and stat["size"] > MIN_DATA_FILE_SIZE
    ):
    ...
    if r:
        ...
        with open(filepath, "wb") as f:
            for chunk in tqdm_prog(
                r.iter_content(chunk_size=chunk_size),
                total=ceil(remote_size / float(chunk_size)),
            ):
    ...
    log.debug(
        "filepath={}: local_size={}, remote_size={}, downloaded_bytes={}".format(
...
def download_name(name, verbose=True, **kwargs):
    ...
    return download_file(
        url=url, size=size, verbose=verbose, normalize_filename=True, **kwargs
    )
...
def read_named_csv(name, data_path=DATA_PATH, nrows=None, verbose=True):
    ...
    try:
        return KeyedVectors.load_word2vec_format(
            os.path.join(BIGDATA_PATH, name + ".bin.gz"), binary=True
        )
    except IOError:
        pass
    except ValueError:
...
def get_data(name="sms-spam", nrows=None, limit=None):
    ...
    if name in BIG_URLS:
        ...
        filepath = (
            filepaths[name][0]
            if isinstance(filepaths[name], (list, tuple))
            else filepaths[name]
        )
        ...
        if filepathlow.endswith(".w2v.txt"):
            try:
                return KeyedVectors.load_word2vec_format(
                    filepath, binary=False, limit=nrows
                )
            except (TypeError, UnicodeError):
                pass
        if (
            filepathlow.endswith(".w2v.bin")
            or filepathlow.endswith(".bin.gz")
            or filepathlow.endswith(".w2v.bin.gz")
        ):
            try:
                return KeyedVectors.load_word2vec_format(
                    filepath, binary=True, limit=nrows
                )
            except (TypeError, UnicodeError):
                pass
...
def multifile_dataframe(
    paths=["urbanslang{}of4.csv".format(i) for i in range(1, 5)],
    header=0,
    index_col=None,
):
    ...
    for p in paths:
        df = df.append(
            read_csv(p, header=header, index_col=index_col),
...
def clean_toxoplasmosis(
    url="http://www.rightdiagnosis.com/t/toxoplasmosis/stats-country.htm",
):
    dfs = pd.read_html(
        "http://www.rightdiagnosis.com/t/toxoplasmosis/stats-country.htm", header=0
    )
    df = dfs[0].copy()
    ...
    df["frequency"] = (
        df.extrapolated_prevalence.astype(float) / df.population_estimated_used
    )
    return df
    ...
    for c in df.columns:
        ...
        if df[c].dtype.char in "<U S O".split():
            try:
                ...
                if values.str.len().sum() > 0.2 * df[c].astype(str).str.len().sum():
        ...
        if values is not None:
            if values.isnull().sum() < 0.6 * len(values) and values.any():
                df[c] = values
    return df
...
def load_geo_adwords(filename="AdWords API Location Criteria 2017-06-26.csv.gz"):
    ...
    canonical = pd.DataFrame(
        [list(row) for row in df.canonical_name.str.split(",").values]
    )
...
def clean_cornell_movies(
    filename="cornell_movie_dialogs_corpus.zip", subdir="cornell movie-dialogs corpus"
):
    ...
    if fullpath_zipfile.lower().endswith(".zip"):
        ...
        dirname = dirname[:-4]
    fullpath_movie_lines = os.path.join(
        BIGDATA_PATH, dirname, subdir, "movie_lines.txt"
    )
    dialog = pd.read_csv(
        fullpath_movie_lines,
        sep=r"\+\+\+\$\+\+\+",
        engine="python",
        header=None,
        index_col=0,
...
def nlp(texts, lang="en", linesep=None, verbose=True):
    ...
    tqdm_prog = (
        no_tqdm
        if (not verbose or (hasattr(texts, "__len__") and len(texts) < 3))
        else tqdm
    )
    global _parse
    if not _parse:
        try:
            ...
            try:
                ...
                log.warning(
                    "Unable to download Spacy language model '{}' so nlp(text) just returns text.split()".format(
                        lang
                    )
                )
...
def clean_win_tsv(
    filepath=os.path.join(DATA_PATH, "Products.txt"),
    index_col=False,
    sep="\t",
    lineterminator="\r",
    ...
    **kwargs,
    ...
    df[index_col] = (
        df[index_col].str.strip().apply(lambda x: x if x else str(INT_MIN)).astype(int)
    )
    ...
    if len(df) != original_len:
        log.warning(
            (
                'Loaded {} rows from tsv. Original file, "{}", contained {} seemingly valid lines.'
                + "Index column: {}"
            ).format(len(df), original_len, filepath, index_col)
        )
    return df
